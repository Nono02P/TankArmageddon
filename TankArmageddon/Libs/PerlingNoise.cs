...
namespace TankArmageddon
{
    public class PerlinNoise
    {
        public static float[] Generate1DMap(int pMapSize, float pScale)
        {
            ...
            {
                float sampleX = offset + x / pScale;
                result[x] = Simplex.Generate(sampleX);
            }
            return result;
        }

        public static float[] Generate2DMap(Vector2 pMapSize, float pScale)
        {
            if (pScale <= 0)
            {
                pScale = 0.00001f;
            ...
            for (ushort x = 0; x < pMapSize.X; x++)
            {
                for (ushort y = 0; y < pMapSize.Y; y++)
                {
                    float sampleX = x / pScale;
                    float sampleY = y / pScale;
                    result[x + y * (uint)(pMapSize.X)] = Simplex.Generate(sampleX, sampleY);
                }
            }
            return result;
        }
